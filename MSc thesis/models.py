...
class create_classifier(nn.Module):
    ...
    def __init__(self, latent_dim, nhidden=16, N=4):
        super(create_classifier, self).__init__()
        self.gru_rnn = nn.GRU(latent_dim, nhidden, batch_first=True).double()
        self.classifier = nn.Sequential(
            nn.Linear(nhidden, 300),
            nn.ReLU(),
            nn.Linear(300, 300),
            nn.ReLU(),
            nn.Linear(300, N)).double()
...
class multiTimeAttention(nn.Module):
    ...
    def __init__(self, input_dim, nhidden=16, 
        ...
        self.linears = nn.ModuleList([nn.Linear(embed_time, embed_time), 
                                      ...
                                      nn.Linear((input_dim*num_heads), nhidden).double()])
    ...
    def forward(self, query, key, value, mask=None, dropout=None):
        ...
        x = x.transpose(1, 2).contiguous() \
             .view(batch, -1, self.h * dim).type('torch.DoubleTensor').to(
                 torch.device(
...
class enc_mtan_rnn(nn.Module):
    def __init__(self, input_dim, query, latent_dim=2, nhidden=16, 
                 embed_time=16, num_heads=1, learn_emb=False, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
        ...
        self.gru_rnn = nn.GRU(nhidden, nhidden, bidirectional=True, batch_first=True).double()
        self.hiddens_to_z0 = nn.Sequential(
            nn.Linear(2*nhidden, 50),
            nn.ReLU(),
            nn.Linear(50, latent_dim * 2)).double()
        if learn_emb:
    ...
    def learn_time_embedding(self, tt):
        ...
        tt = tt.unsqueeze(-1)
        #print(self.periodic.dtype)
        #print(self.linear.dtype)
    ...
    def fixed_time_embedding2(self, pos):
        d_model=self.embed_time
        pe = torch.zeros(pos.shape[0], pos.shape[1], d_model)
    ...
    def forward(self, x, time_steps):
        ...
        if self.learn_emb:
            key = self.learn_time_embedding(time_steps.type('torch.FloatTensor')).to(self.device)
            query = self.learn_time_embedding(self.query.unsqueeze(0)).to(self.device)
        else:
            key = self.fixed_time_embedding2(time_steps).to(self.device)
            query = self.fixed_time_embedding2(self.query.unsqueeze(0)).to(self.device)
...
class dec_mtan_rnn(nn.Module):
    ...
    def __init__(self, input_dim, query, latent_dim=2, nhidden=16, 
        ...
        self.gru_rnn = nn.GRU(latent_dim, nhidden, bidirectional=True, batch_first=True).double()   
        self.z0_to_obs = nn.Sequential(
            nn.Linear(2*nhidden, 50),
            nn.ReLU(),
            nn.Linear(50, input_dim)).double()
        if learn_emb:
            self.periodic = nn.Linear(1, embed_time-1)
            self.linear = nn.Linear(1, 1)
    ...
    def learn_time_embedding(self, tt):
        tt = tt.to(self.device)
        tt = tt.unsqueeze(-1)
        tt = tt.type('torch.FloatTensor').to(self.device)
        out2 = torch.sin(self.periodic(tt))
        out1 = self.linear(tt)
        return torch.cat([out1, out2], -1)
    ...
    def forward(self, z, time_steps):
        ...
        if self.learn_emb:
            ...
            query = self.fixed_time_embedding2(time_steps.type('torch.FloatTensor')).to(self.device)
            key = self.fixed_time_embedding2(self.query.unsqueeze(0).type('torch.FloatTensor')).to(self.device)
